...
public class Localization {
    ...
    BufferedReader reader = new BufferedReader(new InputStreamReader(Game.class.getResourceAsStream(localizationFiles.getOrDefault(selectedLanguage, "/localization/english_en-us.mcpl")), StandardCharsets.UTF_8));
		...
		try {
			...
			if (src != null) {
				...
				while(true) {
					...
					if (name.startsWith("localization/") && name.endsWith(".mcpl")) {
						String data = name.replace("localization/", "").replace(".mcpl", "");
	...
	private static String[] getLanguagesFromDirectoryUsingIDE() {
		...
		try {
			URL fUrl = Game.class.getResource("/localization/");
