...
public class Network extends Game {
	...
	public static boolean getLatestVersionCheck() { return false; }
	...
	public static void findLatestVersion(Action callback) {
		new Thread(() -> {
			...
			try {
				// TODO Add QU repo for release check below
				HttpResponse<JsonNode> response = Unirest.get("https://api.github.com/repos/qbituniverse/qbitcraft/releases").asJson();
	...
	public static void startMultiplayerServer() {
		...
		server = new QbitcraftServer();
		
		new Load(WorldSelectDisplay.getWorldName(), server); // load server config
		
		if(getLatestVersionCheck() && latestVersion == null) {
			...
			findLatestVersion(() -> {
				if(latestVersion.version.compareTo(Game.VERSION) > 0) // link new version
					...
					System.out.println("VERSIONCHECK: Found newer version: Version " + latestVersion.releaseName + " Available! Download direct from \""+latestVersion.releaseUrl +"\". Can also be found with change log at \"https://github.com/qbituniverse/qbitcraft/releases\".");
