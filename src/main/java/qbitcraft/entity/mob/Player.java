...
import qbitcraft.entity.ClientTickable;
...
import qbitcraft.entity.furniture.Tnt;
...
import qbitcraft.screen.CraftingDisplay;
import qbitcraft.screen.InfoDisplay;
import qbitcraft.screen.LoadingDisplay;
import qbitcraft.screen.PauseDisplay;
import qbitcraft.screen.PlayerInvDisplay;
		...
		if (activeItem != null) {
			...
			if (t.x >= 0 && t.y >= 0 && t.x < level.w && t.y < level.h) { // if the target coordinates are a valid tile...
				...
				if (Game.isValidServer() && this instanceof RemotePlayer) {// only do this if no interaction was actually made; b/c a tile update packet will generally happen then anyway.
					qbitcraft.network.QbitcraftServerThread thread = Game.server.getAssociatedThread((RemotePlayer)this);
					//if(thread != null)
