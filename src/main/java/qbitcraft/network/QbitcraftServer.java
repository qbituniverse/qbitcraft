...
import qbitcraft.entity.ItemEntity;
import qbitcraft.entity.furniture.Bed;
import qbitcraft.entity.furniture.Chest;
import qbitcraft.entity.furniture.DeathChest;
...
public class QbitcraftServer extends Thread implements QbitcraftProtocol {
	...
	private List<QbitcraftServerThread> threadList = Collections.synchronizedList(new ArrayList<>());
	...
	public QbitcraftServer() {
		super("QbitcraftServer");
		Game.ISONLINE = true;
	...
	public void run() {
		...
		try {
			while (socket != null) {
				QbitcraftServerThread mst = new QbitcraftServerThread(socket.accept(), this);
				if(mst.isConnected())
					threadList.add(mst);
			}
	...
	private QbitcraftServerThread[] getThreads() {
		return threadList.toArray(new QbitcraftServerThread[threadList.size()]);
	}
	
	public String[] getClientInfo() {
		List<String> playerStrings = new ArrayList<>();
		for(QbitcraftServerThread serverThread: getThreads()) {
	...
	public List<RemotePlayer> getPlayersInRange(Level level, int xt, int yt, boolean useTrackRange) {
		...
		for(QbitcraftServerThread thread: getThreads()) {
	...
	@Nullable
	public QbitcraftServerThread getAssociatedThread(String username) {
		QbitcraftServerThread match = null;
	...
	public List<QbitcraftServerThread> getAssociatedThreads(String[] usernames) { return getAssociatedThreads(usernames, false); }
	public List<QbitcraftServerThread> getAssociatedThreads(String[] usernames, boolean printError) {
		List<QbitcraftServerThread> threads = new ArrayList<>();
		for(String username: usernames) {
			QbitcraftServerThread match = getAssociatedThread(username);
			if(match != null)
				threads.add(match);
			else if(printError)
	...
	@NotNull
	public QbitcraftServerThread getAssociatedThread(RemotePlayer player) {
		QbitcraftServerThread thread = null;
		
		for(QbitcraftServerThread curThread: getThreads()) {
		...
		if(thread == null) {
			...
			//Thread.dumpStack();
			thread = new QbitcraftServerThread(player, this);
		}
		
		return thread;
	}
	
	private List<QbitcraftServerThread> getAssociatedThreads(List<RemotePlayer> players) {
		List<QbitcraftServerThread> threads = new ArrayList<>();
		...
		for(RemotePlayer client: players) {
			QbitcraftServerThread thread = getAssociatedThread(client);
			if(thread.isValid())
				threads.add(thread);
			else
	...
	public void broadcastEntityUpdate(Entity e, boolean updateSelf) {
		...
		for(QbitcraftServerThread thread: getAssociatedThreads(players)) {
	...
	public void updateGameVars(QbitcraftServerThread sendTo) {
		updateGameVars(new QbitcraftServerThread[] {sendTo});
	}
	public void updateGameVars(QbitcraftServerThread[] sendTo) {
		...
		for(QbitcraftServerThread thread: sendTo)
	...
	boolean parsePacket(QbitcraftServerThread serverThread, InputType inType, String alldata) {
	...
	private void broadcastData(InputType inType, String data) {
		broadcastData(inType, data, (QbitcraftServerThread)null);
	}
	private void broadcastData(InputType inType, String data, @Nullable QbitcraftServerThread clientThreadToExclude) {
		for(QbitcraftServerThread thread: getThreads()) {
			if(thread != clientThreadToExclude) // send this packet to all EXCEPT the specified one.
				thread.sendData(inType, data);
		}
	}
	private void broadcastData(InputType inType, String data, List<QbitcraftServerThread> threads) {
		for(QbitcraftServerThread thread: threads)
			thread.sendData(inType, data);
	}
	
	protected synchronized void onThreadDisconnect(QbitcraftServerThread thread) {
	...
	public synchronized void endConnection() {
		...
		if(hasClients()) {
			...
			QbitcraftServerThread[] threads = getThreads();
			for(QbitcraftServerThread thread: threads)
