...
public abstract class QbitcraftConnection extends Thread implements QbitcraftProtocol {
	
	private PrintWriter out;
	private BufferedReader in;
	private Socket socket = null;
	
	protected QbitcraftConnection(String threadName, @Nullable Socket socket) {
		super(threadName);
		this.socket = socket;
	...
	public void run() {
		...
		while(isConnected()) {
			...
			if(read > 0) { // if it is valid character that is not the null character, then add it to the string.
				...
				InputType inType = QbitcraftProtocol.getInputType(currentData.charAt(0));
