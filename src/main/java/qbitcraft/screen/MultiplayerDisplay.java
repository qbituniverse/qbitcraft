...
import qbitcraft.gfx.Ellipsis.SequentialEllipsis;
import qbitcraft.gfx.Font;
import qbitcraft.gfx.FontStyle;
import qbitcraft.gfx.Screen;
import qbitcraft.network.QbitcraftClient;
import qbitcraft.saveload.Save;
import qbitcraft.screen.entry.RangeEntry;
...
public class MultiplayerDisplay extends Display {
	...
	private static String domain = "https://playqbitcraft.com";
	...
	private final RangeEntry connectTimeout = new RangeEntry("Timeout (sec) (0=none)", 0, 120, (int)Math.ceil(QbitcraftClient.DEFAULT_CONNECT_TIMEOUT/1000f));
	...
	public MultiplayerDisplay(String ipAddress) {
		...
		contactAccountServer(() -> {
			if(curState == State.ENTERIP) { // login was automatic
				...
				Game.client = new QbitcraftClient(savedUsername,this, ipAddress);
			} else
		...
		Unirest.get(domain).asBinaryAsync(new Callback<InputStream>() {
			...
			public void completed(HttpResponse<InputStream> httpResponse) {
				if(httpResponse.getStatus() == 200)
					...
					System.err.println("warning: qbitcraft site ping returned status code " + httpResponse.getStatus());
			...
			public void cancelled() {
				...
				if(savedUsername.length() == 0 || savedUUID.length() == 0) {
					...
					setError("could not connect to qbitcraft account server, but no login data saved; cannot enter offline mode.", false);
					return;
				}
		...
		switch(curState) {
			...
			case ENTERIP:
				...
				if(input.getKey("select").clicked) {
					...
					savedIP = typing;
					new Thread(() -> {
						Game.client = new QbitcraftClient(savedUsername, this, typing, connectTimeout.getValue()*1000); // typing = ipAddress
	...
	private void fetchName(String uuid) {
		...
		if(response != null) {
			...
			switch(json.getString("status")) {
				...
				case "success":
					if(Game.debug) System.out.println("successfully received username from qbitcraft server");
					savedUsername = json.getString("name");
					break;
			}
		} else// if(Game.debug)
